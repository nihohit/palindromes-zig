...
fn get_mirrors(num: u128) error{ NoSpaceLeft, InvalidCharacter, Overflow }![2]u128 {
    var mirrored_buf = [_]u8{0} ** 128;
    const slice = try std.fmt.bufPrint(&mirrored_buf, "{d}", .{num});
    for (slice, 0..slice.len) |s, i| mirrored_buf[2 * slice.len - i - 1] = s;
    const first_num = try std.fmt.parseUnsigned(u128, mirrored_buf[0 .. slice.len * 2], 10);

    for (slice, 0..slice.len) |s, i| mirrored_buf[2 * slice.len - i - 2] = s;
    const second_num = try std.fmt.parseUnsigned(u128, mirrored_buf[0 .. slice.len * 2 - 1], 10);
    return [2]u128{ first_num, second_num };
}

fn check_if_binary_palindrome(num: u128) error{NoSpaceLeft}!bool {
    var binary_buf = [_]u8{0} ** 130;
    var binary_slice = try std.fmt.bufPrint(&binary_buf, "{b}", .{num});
    return check_if_slice_is_palindrome(binary_slice);
}

fn check_and_print(num: u128) error{NoSpaceLeft}!void {
    if (try check_if_binary_palindrome(num)) {
        std.debug.print("Found {d}\n", .{num});
    }
}

fn check_if_palindrome(num: u128) error{NoSpaceLeft}!bool {
    var buf = [_]u8{0} ** 30;
    var slice = try std.fmt.bufPrint(&buf, "{d}", .{num});
    return check_if_slice_is_palindrome(slice) and try check_if_binary_palindrome(num);
}

fn mirror(num: u128, expected_length: u32) error{ NoSpaceLeft, InvalidCharacter, Overflow }!u128 {
    var mirrored_buf = [_]u8{0} ** 60;
    const slice = try std.fmt.bufPrint(&mirrored_buf, "{d}", .{num});
    for (slice, 0..slice.len) |s, i| mirrored_buf[expected_length - i - 1] = s;
    return try std.fmt.parseUnsigned(u128, mirrored_buf[0..expected_length], 10);
}

fn mirror_binary(num: u128, expected_length: u8) u128 {
    return num | @bitReverse(num) >> @truncate(u7, (128 - expected_length));
}

fn mirror_binary_max(num: u128, expected_length: u8, recursion_depth: u8) u128 {
    const reverse = @bitReverse(num);
    const truncated_recursion_depth = @truncate(u7, recursion_depth);
    const max_int: u128 = math.maxInt(u128);
    const only_ones = max_int >> truncated_recursion_depth;
    const reverse_and_ones = (only_ones | reverse) >> @truncate(u7, 128 - expected_length);
...
fn is_pruned(current_digits: u128, decimal_length: u32, recursion_depth: u8) error{ NoSpaceLeft, InvalidCharacter, Overflow }!bool {
    const changing_decimal_length = (decimal_length + 1 - 2 * recursion_depth) / 2;
    const exp = math.pow(u128, 10, changing_decimal_length);
    ...
    const bit_length = 128 - @clz(min_decimal_palindrome);
    const clz = @clz(max_decimal_palindrome);
    if ((128 - clz) != bit_length) {
        return false;
    }
...
fn find_palindrome(current_digits: u128, decimal_length: u32, recursion_depth: u8) error{ NoSpaceLeft, InvalidCharacter, Overflow }!void {
